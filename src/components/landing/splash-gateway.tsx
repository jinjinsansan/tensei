...
export function SplashGateway() {
  ...
  const heroLines = useMemo(() => ['TENSEI', 'GACHA'], []);
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col items-center justify-center px-6 py-10">
        {phase !== 'complete' && (
          <div className="relative flex h-[62vh] w-full items-center justify-center overflow-hidden rounded-[32px] border border-white/10 bg-black/30 shadow-[0_0_45px_rgba(0,0,0,0.5)]">
            ...
            >
              <Image src="/opensonshi.png" alt="TENSEI" fill priority className="object-cover object-top" />
            </motion.div>
            <motion.div
        ...
        {phase !== 'complete' ? (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-8 text-xs uppercase tracking-[0.6em] text-neon-yellow">
            LOADING
          </motion.p>
        ) : (
          <motion.div className="mt-0 w-full max-w-sm space-y-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-white/80">尊師ホールの導線を転生テーマで再構築したネオンガチャへようこそ。</p>
              </div>
            </div>
